...
const App = () => {
  ...
  const handleKeyDown = (e) => {
    ...
    const { key } = e;
    setUserInput(
      (userInput.toString().replace('Shift', '') ||
        userInput.toString().replace('CapsLock', '')) +
        key.toString().replace('Shift', '')
    );
    ...
    if (key === challengeText.charAt(index)) {
      setIndex(index + 1);
      const currentChar = challengeText.substring(
        index + 1,
        index + challengeText.length
      );
      ...
      <div className='container-fluid pt-4'>
          ...
          <div className='col-sm-12 col-md-8 order-md-1'>
            <div className='container border px-5 rounded'>
              <div className='text-center mt-4 header'>
                ...
                <p className='lead'>
                  Start the Typing speed challenge and find out how fast you can
                  type for real!
                </p>
        ...
        {/* <div className={`details-modal ${ displayModal ? 'remove' : ''}`}>
          ...
          <div className="details-modal-content result">
            <ItemList name='Speed' data={lastSpeed} />
            <ItemList name='Accuracy' data={lastAccuracy} symble='%'/>
            <ItemList name='Points' data={lastScore} />
          </div>
        </div> */}
        <Modal wpm={wpm} cwc={lastScore} accuracy={accuracy} displayModal={displayModal} setDisplayModal={setDisplayModal} />
      </div>
    </div>
  );
};
